# TOS Alpha-3: High-Fidelity & AI Integration Roadmap
...
- [ ] **AI Mode Implementation:** Develop the `AiService` in the Brain to handle Natural Language Processing.
- [ ] **Staged Command Generation:** Enable the AI to propose complex shell scripts for user review before execution.
- [ ] **Contextual Awareness:** Feed system state (active sectors, search results, system logs) into the AI context for smarter assistance.
- [ ] **Natural Language Search:** Replace string matching in SEARCH mode with semantic embedding-based retrieval.

## Phase 11: High-Fidelity Rendering & Themes (§9, §10, §18)
- [ ] **Web Profile DOM Implementation:** Transition the Face layer from terminal-mock to a rich React/Vite-based LCARS interface.
- [ ] **Cinematic Modules:** Implement the "Cinematic Triangular Module" with GPU-accelerated transitions.
- [ ] **Dynamic Theme Engine:** Implement runtime CSS variable injection and multi-sensory asset swapping (SFX/Haptics).
...
- [ ] **Deep Inspection (Level 5) Audit:** Implement a cryptographic audit log for all privilege escalations.
