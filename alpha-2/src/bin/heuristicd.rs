...
async fn main() -> anyhow::Result<()> {
    ...
    println!("TOS-HEURISTICD: Operational on port {}", port);
            ...
            match tokio::net::UnixStream::connect(brain_sock_path).await {
                Ok(mut sock) => {
                    ...
                    if let Err(e) = sock.write_all(register_cmd.as_bytes()).await {
                        eprintln!("[HEURISTICD] Failed registration: {}", e);
                    } else {
                        println!("[HEURISTICD] Registered with Brain on port {}", port);
                    }
                    break;
                }
                Err(_) => {
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = handle_client(socket, state_clone).await {
                eprintln!("[HEURISTICD] Client error: {}", e);
            }
        });
    }
}
