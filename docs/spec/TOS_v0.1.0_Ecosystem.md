...
For system architecture, IPC contracts, and visual hierarchy, see the [Architecture Specification](./TOS_v0.1.0_Architecture.md).
...
**Full AI system specification:** See [Features Specification §4 — Ambient AI & Co-Pilot System](./TOS_v0.1.0_Features.md).
...
- **Render the output** to a surface provided by the Face. See Architecture §27.1 for rendering and interaction contracts.
...
pub trait TerminalOutputModule {
    ...
    // UI-side rendering and event handling defined in Architecture §27.1
}
```
...
Bezel Components are modular UI elements that can be installed via the marketplace and docked into any available Tactical Bezel Slot (Top, Left, or Right). Each component runs as a background process or thread and communicates with the Face via the API defined in Architecture §27.2.
...
Audio themes define earcon sets and ambient audio layers for the three-layer audio model (Visual Design §3). Can be installed via the Marketplace and selected in **Settings → Interface → Audio Theme**.
...
The full Marketplace UI specification — including the Home view, Category Browse, Module Detail Page, and install flow — is documented in [Features Specification §5 — Marketplace Discovery](./TOS_v0.1.0_Features.md).
...
5. **Re-discover:** If a connection drops, the client reconnects to the anchor port and re-queries the map. See [Architecture §3.4.4](./TOS_v0.1.0_Architecture.md#344-reconnection-logic) for the full retry schedule and visual states during reconnection.
