...
impl TrustService {
    ...
    pub fn classify_command(
        ...
        if cmd == "su"
            ...
            || cmd.starts_with("su ")
            || cmd.starts_with("doas ")
            || cmd.starts_with("pkexec ")
        {
            return CommandClass::PrivilegeEscalation;
        }

        let is_destructive = cmd.starts_with("rm ")
            || cmd.starts_with("cp ")
            || cmd.starts_with("mv ")
            || cmd.starts_with("chmod ")
            || cmd.starts_with("chown ");
        let is_recursive =
            cmd.contains(" -r") || cmd.contains(" -R") || cmd.contains(" --recursive");
    ...
    pub fn get_trust_policy(
        ...
        state
            .settings
            .resolve(class_key, sector_id, None)
...
mod tests {
    ...
    fn test_classify_standard() {
        ...
        let cwd = Path::new("/");
        assert_eq!(
            trust.classify_command("ls -la", cwd, 10),
            CommandClass::Standard
        );
        assert_eq!(
            trust.classify_command("cat file.txt", cwd, 10),
            CommandClass::Standard
        );
    }
    ...
    fn test_classify_privilege_escalation() {
        ...
        let cwd = Path::new("/");
        assert_eq!(
            trust.classify_command("sudo apt update", cwd, 10),
            CommandClass::PrivilegeEscalation
        );
        assert_eq!(
            trust.classify_command("su -", cwd, 10),
            CommandClass::PrivilegeEscalation
        );
        assert_eq!(
            trust.classify_command("pkexec reboot", cwd, 10),
            CommandClass::PrivilegeEscalation
        );
    }
    ...
    fn test_classify_recursive_bulk() {
        ...
        let cwd = Path::new("/");
        assert_eq!(
            trust.classify_command("rm -rf /tmp/foo", cwd, 10),
            CommandClass::RecursiveBulk
        );
        assert_eq!(
            trust.classify_command("cp -R a b", cwd, 10),
            CommandClass::RecursiveBulk
        );
    }
    ...
    fn test_classify_implicit_bulk_glob() {
        ...
        assert_eq!(
            trust.classify_command("rm *.txt", path, 3),
            CommandClass::ImplicitBulk
        );

        // Threshold 10 -> Should be Standard
        assert_eq!(
            trust.classify_command("rm *.txt", path, 10),
            CommandClass::Standard
        );
    }
