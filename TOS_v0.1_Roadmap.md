...
| Per-surface timeline (Level 4) | §19.1 | ✅ | `TimelineService` records `TosState` snapshots; `timeline_scrub` IPC enabled |
| OpenSearch compatibility | §19.3 | ✅ | `RemoteServer` serves `opensearch.xml` and `/search` via HTTP-over-TCP |
